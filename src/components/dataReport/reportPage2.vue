  ...
  <div>
    <div class="pager pager01">
        ...
        <div class="panel-head">
          <h1 class="tit-idx">01</h1>
          <div class="tit-dsp">
            <h2>我们的成绩单</h2>
          </div>
        </div>
        <div class="panel-body">
          <div class="sum-tit padding-top-2rem">
            <div class="tit">平台数据统计</div>
            <div class="tit-sub">截止 2017.9.30</div>
          </div>
          <div class="padding-2rem">
           <!-- <table cellspacing="0" cellpadding="0" width="100%">
              <tbody><tr>
                <td width="25%">
                  <div class="pop">
                    <i class="icon iconfont icon-dr-a-turnover"></i>
                  </div>
                </td>
                <td width="75%">
                  <div class="text-h5 font-lighter">累计成交总额(元）</div>
                  <div class="text-orange text-DINOT text-h2 line-height-1 cumulative-turnover count-scroll" :data-val="page2_data.totalInvestMoney">
                    <span v-for="item in toThousands(page2_data.totalInvestMoney)"  :class="parseInt(item)>=0?'num':'num-dot'">
              ...
              <tr>
                <td width="25%">
                  <div class="pop">
                    <i class="icon iconfont icon-dr-user"></i>
                  </div>
                </td>
                <td width="75%">
                  <div class="text-h5 font-lighter">累计用户数(人）</div>
                  <div class="text-orange text-DINOT text-h2 line-height-1 cumulative-user count-scroll" data-val="1321036">
                    <span class="num num-1"><i>0123456789</i></span>
                    <span class="num-dot active"><i>,</i></span>
                    <span class="num num-3"><i>0123456789</i></span>
                    <span class="num num-2"><i>0123456789</i></span>
                    <span class="num num-1"><i>0123456789</i></span>
                    <span class="num-dot active"><i>,</i></span>
                    <span class="num num-0"><i>0123456789</i></span>
                    <span class="num num-3"><i>0123456789</i></span>
                    <span class="num num-6"><i>0123456789</i></span>
                  </div>
                </td>
              </tr>
              <tr>
                <td width="25%">
                  <div class="pop">
                    <i class="icon iconfont icon-dr-profit"></i>
                  </div>
                </td>
                <td width="75%">
                  <div class="text-h5 font-lighter">累计为用户赚取收益(元）</div>
                  <div class="text-orange text-DINOT text-h2 line-height-1 cumulative-profit count-scroll" data-val="161880837.10">
                    <span class="num num-1"><i>0123456789</i></span>
                    <span class="num num-6"><i>0123456789</i></span>
                    <span class="num num-1"><i>0123456789</i></span>
                    <span class="num-dot active"><i>,</i></span>
                    <span class="num num-8"><i>0123456789</i></span>
                    <span class="num num-8"><i>0123456789</i></span>
                    <span class="num num-0"><i>0123456789</i></span>
                    <span class="num-dot active"><i>,</i></span>
                    <span class="num num-8"><i>0123456789</i></span>
                    <span class="num num-3"><i>0123456789</i></span>
                    <span class="num num-7"><i>0123456789</i></span>
                    <span class="num-dot active"><i>.</i></span>
                    <span class="num num-1"><i>0123456789</i></span>
                    <span class="num num-0"><i>0123456789</i></span>
                  </div>
            ...
            <table cellspacing="0" cellpadding="0" width="100%">
              ...
              <tr>
                ...
                <td width="75%">
                  <div class="text-h5 font-lighter">累计用户数(人）</div>
                  <div class="text-orange text-DINOT text-h2 line-height-1 cumulative-user count-scroll" :data-val="page2_data.totalMember">
                    <span v-for="item in toThousands(page2_data.totalMember)"  :class="parseInt(item)>=0?'num':'num-dot'">
                        <i v-if="parseInt(item)>=0">0123456789</i>
                        <i v-else>{{item}}</i>
                    </span>
                  </div>
              ...
              <tr>
                ...
                <td width="75%">
                  <div class="text-h5 font-lighter" >累计为用户赚取收益(元）</div>
                  <div class="text-orange text-DINOT text-h2 line-height-1 cumulative-profit count-scroll" :data-val="page2_data.totalIncome">
                    <span v-for="item in toThousands(page2_data.totalIncome)"  :class="parseInt(item)>=0?'num':'num-dot'">
                        <i v-if="parseInt(item)>=0">0123456789</i>
                        <i v-else>{{item}}</i>
                    </span>
                  </div>
...
<style lang="less" scoped>
  ...
  @font-face {
    font-family: 'DINOT';
    src:url('http://cdn.renrenaijia.com/CDN/app/Public/images/kmjrAppV3/dataReport/201704/DINOT-Bold.ttf/DINOT-Bold.ttf') format('truetype');
  }
  *{padding: 0;margin: 0; font-family: "苹方", "Helvetica Neue", Helvetica, Arial, sans-serif }
  body,html{ background-color: #fff;color: #646464; display: block;width: 100%; height: 100%; overflow: hidden}
  ...
  .panel-head h1{ font-weight: lighter; font-size: 4rem; line-height: 1; float: left; font-weight: 300; color: #fd6a53; position: relative}
  ...
  .list-rank{ border-radius: .25rem; background-color: #f9f9f9; color: #666666; margin:.25rem 2rem 0; padding: .25rem 0; text-align: center }
  .list-rank.list-tit{ background-color: transparent; color: #333}
  .list-cell{ font-size: .875rem}
  ...
  .list-top{ position: relative; height: 14rem;
    /* padding-bottom: 1rem*/}

  .list-top-cell{ position: absolute;width: 24%; height: 7.6rem; left:37%; top: 0; background: url('http://cdn.renrenaijia.com/CDN/app/Public/images/kmjrAppV3/dataReport/201704/page07-top-bg.png') left top no-repeat; background-size: 100% 100%; }
  .list-top-cell:nth-child(2){ top: 6.1rem; left: 24.5%}
  .list-top-cell:last-child{ top: 6.1rem; left:49.5%}
  ...
  .t-box .integral{text-align: center; font-size: 1.75rem; font-weight: bold; color: #fd6a53; line-height: 1.2}
  ...
  .pop.pop-yellow,
  .pop.pop-blue,
  .pop.pop-green{box-shadow:0 0 .5rem #fc573d; line-height: 1.25rem; padding-top: 2rem; height: 4.5rem; width: 6.5rem; position: relative; margin: 0 auto; border: 0}
  .pager10 .pop{ height: 6rem; padding-top:2.5rem; width: 8.5rem}
  ...
  .pop .crown{ position: absolute; width: 2rem; height: 2rem; left: 2.375rem; top: -.875rem; background-color: white; border-radius: 999rem; box-shadow: 0 0 .25rem #fc573d; z-index: 2}
  .pop .crown .icon{ color: #fc573d; font-size: 1.25rem}
  .pager10 .pop .crown{left: 3.125rem; top: -.875rem;width: 2.5rem; height: 2.5rem;}
  ...
  .slider{ width:80%; padding:5% 5% 0; margin: 2rem auto; position: relative; z-index: 1; min-height: 19rem; box-shadow: 0 0  .25rem #f1eeee; overflow: hidden}
  .slider img{ display: block;}
  .slider-info{ padding: 1rem 0 0}
  ...
  .slider-items{ position: absolute; left: 5%;  top: 5%;  z-index: 1;transition: all .5s; opacity: 0; width: 90%; }
  ...
  .app-link .icon{ display: inline-block; transform: rotate(-90deg);-webkit-transform: rotate(-90deg)}
  /*****/
